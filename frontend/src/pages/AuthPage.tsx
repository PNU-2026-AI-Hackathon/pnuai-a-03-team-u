    ...
    <main className="auth-page">
      ...
      <section className="auth-card" data-auth-mode={mode}>
        ...
        <form className={`auth-form${mode === "login" ? " active" : ""}`} onSubmit={handleLogin}>
          ...
          <label>
            <span>이메일</span>
            <input type="email" placeholder="이메일을 입력하세요" value={loginEmail} onChange={(event) => setLoginEmail(event.target.value)} required />
          </label>
          <label>
            <span>비밀번호</span>
            <input type="password" placeholder="비밀번호를 입력하세요" value={loginPassword} onChange={(event) => setLoginPassword(event.target.value)} required />
          </label>
