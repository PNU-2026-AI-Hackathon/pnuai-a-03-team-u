...
- **이도원**
  ...
  >
  > 가장 무거웠던 건 팀원이 만든 졸업요건 스키마를 통째로 되돌리기로 한 결정이었다. 이미 여러 PR이 그 위에 쌓여 있었는데, 골든테스트를 통과시키려 할수록 설계가 복잡해지기만 했다. 만든 사람에게 "이거 되돌리자"고 꺼내는 게 제일 어려운 부분이었다 — 결국 말로 설득하기보다 실패하는 테스트 케이스를 같이 보여주고 나서야 납득이 됐다. 더 늦게 되돌릴수록 설득 비용도 같이 커진다는 걸 그때 배웠다.
