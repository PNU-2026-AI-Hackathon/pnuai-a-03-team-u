...
- **수집 최소화 및 동의(미도입 — 배포 이후에도 아직 없음)**: 서비스 제공에 필요한 항목만 수집한다는 원칙은 지키고 있지만, 가입 시 사전 동의를 받는 절차 자체가 프론트엔드에 아직 구현돼 있지 않다. "배포 전에 도입한다"고 적어뒀던 항목인데 이미 배포된 상태에서도 빠져 있는 것이라, 실사용자 확대 전 최우선으로 처리해야 한다.
