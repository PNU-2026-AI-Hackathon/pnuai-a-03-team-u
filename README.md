...
- **수집 최소화 및 동의**: 서비스 제공에 필요한 항목만 수집한다. 회원가입에 필수 체크박스로 동의를 받고(서버가 재검증, 체크 안 하면 API를 직접 호출해도 422로 거부) `users.privacy_consent`/`privacy_consent_at`에 기록한다. 온보딩 화면의 옛 "✓ 개인정보 수집 · 이용 동의" 문구는 실제로는 응답을 받지 않는 표시 텍스트였던 걸 확인하고 정리했다. **마이그레이션은 아직 팀 공유 Supabase에 반영 전** — 반영 전까지는 기존 가입자의 `privacy_consent`가 `false`로 남는다.
...
- **개인정보처리방침 · LLM 처리위탁 고지**: `/privacy` 페이지에 수집 항목·목적·보유기간·OpenAI/Langfuse 위탁 범위를 실제 코드 감사 결과 그대로 명시했다. 회원가입 동의 체크박스에서 링크로 연결된다.
...
| F-08 | 개인정보처리방침 · LLM 처리위탁 고지 | ✅ 구현 완료(2026-08-24) — 수집 항목·보유기간·파기 절차·OpenAI/Langfuse 위탁 범위를 `/privacy` 페이지에 명시, 회원가입 필수 동의 체크박스(서버 재검증 포함)로 연결. **단, 이 마이그레이션은 아직 팀 공유 Supabase에 반영 전** — 반영 전까지 기존 가입자의 동의 기록은 없다 |
