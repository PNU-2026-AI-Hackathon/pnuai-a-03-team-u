...
> **한눈에 보기** — 이수내역을 바탕으로 **① 졸업요건을 규칙 기반으로 판정**하고, **② 졸업까지의 학기별 로드맵**과 **③ 다음 학기의 시간 충돌 없는 시간표 후보**를 제안합니다. AI는 판정 결과를 바꾸지 않으며, 사용자가 승인한 제안만 저장합니다.

| 규칙 기반 판정 | AI는 제안만 | 실제 개설 강좌 기반 |
|:---:|:---:|:---:|
| 졸업요건 충족·미충족을 결정론적으로 계산 | 로드맵·시간표 선택지를 설명하고 추천 | 수강편람의 분반·시간 정보를 바탕으로 후보 생성 |

**바로 사용하기**: [배포 서비스 열기](https://planu-pnu.netlify.app) · [API 문서 보기](https://pnuai-a-03-team-u-production.up.railway.app/docs)
...
| LangChain + OpenAI SDK | 도구 호출·재시도·모델 스왑이 표준화돼 두 에이전트를 같은 패턴으로 구현할 수 있었다. | LLM 에이전트 도구 호출 루프, RAG 임베딩 생성 |
| Langfuse | 비결정적인 LLM 응답의 trace·지연시간·비용을 한곳에서 봐야 했다. | 로드맵/시간표 챗 실행마다 도구 호출·지연시간·비용 기록 |
| slowapi | FastAPI에 기본 내장이 없는 로그인·챗 엔드포인트 요청량 제한이 필요했다. | 엔드포인트별 레이트 리밋(로그인·챗 등) |
...
| 단계 | 원칙 | 적용 방식 |
|---|---|---|
| 수집 | 필요한 항목만, 동의 후에만 (적용됨) | 회원가입 시 필수 체크박스로 동의를 받는다(서버가 재검증하므로 체크 안 하면 API를 직접 호출해도 422로 거부). `users.privacy_consent`/`privacy_consent_at`에 기록. 온보딩 화면의 옛 "✓ 개인정보 수집·이용 동의" 문구가 실제로는 응답을 받지 않는 표시 텍스트였던 걸 확인하고 정리했다. |
| 저장 | 포털 자격증명은 저장하지 않음 | 크롤링에 쓰는 학생지원시스템 학번·비밀번호는 동기화 요청 처리에만 사용하고 서버에 남기지 않는다. 추후 정보화본부와 협의해 크롤링을 학지시 API 연동으로 전환할 계획. |
| 저장 | 비밀번호는 단방향 해시로만 | bcrypt로 해시해 저장 — 평문·역가역 암호화 금지. |
...
|ID|기능명|상태|내용|
|:---:|:---|:---:|:---|
| F-08 | 개인정보처리방침 · LLM 처리위탁 고지 | ✅ 배포 완료 | `/privacy`에 수집 항목·보유기간·파기 절차·OpenAI/Langfuse 위탁 범위를 명시하고, 회원가입 필수 동의 체크박스(서버 재검증 포함)로 연결 |
