...
- **[fix] 실패에 로그를 남긴다.** 재시도 시 warning, 최종 실패 시 "이번 동기화에서는
  상담 여부를 갱신하지 않는다(기존 값 유지)" warning. 헤더만 있고 행이 0인 경우도 info로
  남긴다 — 관측된 간헐 실패가 이 형태였을 가능성을 배제하지 못해서, 재발하면 근거가 남게.
- 테스트 7건 추가. 재시도 관련 3건은 수정 전 코드에서 실패하는 것까지 확인했다.
