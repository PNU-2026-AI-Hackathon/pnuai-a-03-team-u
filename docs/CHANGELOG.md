...
- **로드맵↔시간표 관계**: `timetable_chat.py` 파일 docstring이 이미 명시하고 있었다 —
  ...
  데이터 연결**은 시간표 화면의 "담기" 버튼(`POST /{roadmap_id}/timetable/apply`,
...
갈라지게 재구성, TT→RM "담기" 를 점선 단방향 화살표로 별도 표시. 3.2는 RULE에서 RM/TT
서브그래프가 동시에 갈라지고 각자 독립적으로 APPROVE→RESULT에 합류하도록 재구성(번호도
④/④', ⑤/⑤'로 병렬임을 표시), "담기" 점선 화살표 추가. "SW융합"→"AI융합트랙", 비밀번호
찾기 문구 정정(README 219행, 321행 SMTP 설명도 같이).
