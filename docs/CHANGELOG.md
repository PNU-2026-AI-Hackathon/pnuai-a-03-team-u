...
- **[feat] `student_course_records.substitutes_course_id` 추가**(마이그레이션 `70d591f9bd02`).
  전적대 과목 행이 "이건 PNU ○○를 대체했다"를 가리킨다.
...
- **[feat] API**: `PATCH /me/course-records/{id}/substitution`(지정/해제, 멱등),
  `GET /me/course-records` 응답에 `is_transfer_credit`/`substitutes_course_*` 추가.
  대체 대상 검색은 기존 `GET /courses/search`를 그대로 쓴다.
- **[feat] 화면**: 내 정보 → 학기별 성적 → "입학 전 인정 학점" 그룹의 각 과목 행에
  "어떤 과목을 대체했나요?". 통보를 나중에 받거나 잘못 골라도 언제든 변경/해제할 수 있다.
