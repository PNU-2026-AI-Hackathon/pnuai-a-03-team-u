...
- **2.3 보안**: 볼드 처리가 들쭉날쭉한 플랫 불릿 6개로 나열돼 있던 원칙을 데이터
  생애주기(수집→저장→전송→접근통제→외부전달→보관·파기→투명성) 표로 재구성. "알려진
  한계"가 4개 항목을 `·`로 이어붙인 한 문장이던 것도 불릿으로 분리.
- **2.2 선정 이유 및 역할**: "왜 이걸 골랐나" 열이 항목당 2~3문장이던 걸 한 문장으로
  압축.
- 압축 과정에서 정보가 실제로 빠지는 실수를 했다 — LangChain/OpenAI SDK 행에서
  "임베딩은 LangChain 없이 OpenAI SDK 직접 호출" 근거, Langfuse 행에서 자체 호스팅
  이유, slowapi 행에서 막는 구체 위협(로그인 brute force·LLM 비용 폭탄)이 빠졌었다.
  `/code-review 251`로 3차례 재리뷰를 돌리며 잡아서 복원했다. 그 외에 압축 중 생긴
  "역가역"(비존재 단어, "가역"의 오타)도 리뷰에서 잡혀 수정.
- 같은 PR 브랜치에 이전 세션에서 만들어둔 미커밋 변경(최상단 "한눈에 보기" 요약
  배너·설문조사 결과 이미지 2장·3.2 서비스 체험 안내·F-08/F-09를 "확장 기능"에서
  "운영·개인정보 보호 기능" 표로 분리)이 남아 있어 같이 커밋했다 — PR 설명에 범위를
  정확히 반영해뒀다.
- 리뷰 중 로컬 작업 디렉터리가 세션 밖에서(다른 터미널/세션 추정) `main`으로 전환되고
  PR #250이 병합된 걸 발견했다. 이 세션(코드리뷰 서브에이전트 포함)이 한 일이 아님을
  트랜스크립트로 확인 — `git show <ref>:path`만 쓰고 checkout/merge는 안 한 것으로
  확인됨. 이 PR(#251) 브랜치·커밋은 영향 없었음.

## 2026-08-24 (blackest21) — 인시던트: 2026-2학기 수강편람 66%가 시간 정보 없이 적재돼 있던 것 → 재크롤링으로 복구

**발견 경위**: 한고은님이 실제로 시간표 화면에서 "관리회계(서민근)"·"AI비즈니스(홍태호)" 두
과목을 담아 쓰다가 주간 그리드에 안 뜨는 걸 먼저 발견해서 제보. 이 세션은 그 제보를 받고
`course_offerings`를 직접 조회하면서 시작했는데, 두 과목 다 `course_times` 행이 아예 없었다.

**조사**: 혹시 학과별 단일 과목만의 문제인가 싶어 전체를 집계해보니 훨씬 컸다.

| 학기 | 전체 개설강좌 | 시간정보 있음 | 비율 |
|---|---|---|---|
| 2026-1학기 | 4,289 | 4,206 | 98% |
| 2026-2학기 | 4,340 | 1,474 | **34%** |
| 2026 여름계절수업 | 208 | 187 | 90% |

`created_at`을 분 단위로 찍어보니 2026-08-05 배치 안에서도 시각별로 완성도가 들쭉날쭉했다
(06:38 807건 중 428건 vs 06:39 1,133건 중 95건) — 크롤러가 카테고리(1~5)별로 순회하는데,
그 시점 기준으로 카테고리별 공개 여부가 갈렸던 것으로 보였다.

**원인**: 코드 버그가 아니라 **크롤링 시점** 문제였다. 기존 2026-2학기 스냅샷은 `2026-07-28`에
떠서 개강(9월)까지 5주 넘게 남은 시점이었다 — 학교(One-Stop)가 아직 상당수 과목의 강의실·
시간을 확정 공개하지 않은 상태였고, 원본 JSON 응답 자체에 `TIMETABLE_SUMMARY_INFO`가
`null`로 내려왔다(파싱 실패가 아니라 파싱할 대상 자체가 없었음 — 크롤러(`onestop_course_catalog.py:409`)는
이 경우를 정직하게 `timetable_parse_status="empty"`로 태깅하고, 파서(`onestop_course_catalog.py:81~84`)도
빈 입력이면 그냥 빈 리스트를 반환할 뿐 조용히 삼키는 except/pass는 없었다. 그 스냅샷 전체
행의 `enrolled_count/capacity`가 `0/N`이었다는 것도 "수강신청 시작 전에 너무 일찍 긁었다"는
방증이었다.

**복구**: `app/ingestion/crawlers/onestop_course_catalog.py`로 2026_2 재크롤링(2026-08-24) →
`raw_data/crawled_data/onestop_course_catalog/2026_2/` 스냅샷 갱신 →
`scripts/import_course_offerings.py --commit`으로 Supabase 반영. 결과: 시간정보 보유
1,474/4,340(34%) → 3,455/4,341(80%, 전체 모수가 1 늘어난 건 CSV에 있던 신규 분반 1건이
이번에 같이 생성돼서다 — 엄격한 동일 모수 비교는 아님), 시간 세션 5,202개 신규 파싱, 관리회계·AI비즈니스
둘 다 정상 확인(월/수 13:30~14:45 514-313-1, 월 15:00~18:00 514-313). 코드 변경은 없음 —
데이터만 다시 받아 반영. `course_code` 미매칭 28건(`courses` 테이블에 없는 과목)은 이번
범위 밖이라 그대로 skip.

**재발 방지 메모**: 남은 20%(~886건)는 학교가 여전히 시간표 미확정인 나머지다 — 이번 재크롤링
한 번으로 100%가 되는 문제가 아니라, "다음 학기" 수강편람은 원래 개강이 가까워질수록
채워지는 데이터다. 등록 오픈 훨씬 전에 미리 한 번 긁어두고 그 뒤로 갱신을 안 하면 이런
gap이 재발한다. 다음 학기 준비할 땐 최소 수강신청 기간 즈음에 한 번 더 재크롤링하는 걸
루틴으로 잡아둘 것.

## 2026-08-24 (blackest21) — 로드맵 선수과목 체이닝 버그 + 교양영역 다중선택 + 조사 문서 확장 (PR #222/#223/#224)

사용자가 직접 두 가지를 지적해서 발견·수정. 세 PR 모두 독립 리뷰(worktree) 거쳐 머지
(#224만 문서 전용이라 셀프 리뷰).

- **[fix] #222 — 로드맵 다학기 계획이 앞 학기 계획을 이수 예정으로 안 쳐주던 것.**
  "졸업까지 로드맵 짜줘"로 `propose_term_plan`이 3학년 2학기에 자료구조를 계획해
  넣어도, `_compute_prereq_blocked`는 실제 성적표/전적대 대체인정/`status='completed'`
  로드맵 항목만 이수 예정으로 인정했다 — 아직 실제로 안 들은 `planned` 계획은 물론
  같은 턴의 `pending_changes`(DB 저장 전 제안)도 인정 안 돼서, 4학년 1학기 후속
  과목이 "선수과목 미이수"로 잘못 걸릴 수 있었다. `roadmap_id`가 있을 때 로드맵
  항목 상태 필터를 넓히고(처음엔 `!= 'rejected'`로 잘못 썼다가 독립 리뷰가 잡음 —
  `CourseRoadmapItem.status`는 `planned`/`completed`/`dropped` 셋뿐이고 `rejected`는
  `PendingRoadmapChange.status`의 값이라 이 모델과 무관했다. `!= 'dropped'`로
  수정), 같은 턴의 pending_changes도 함께 이수 예정으로 넘긴다. timetable 챗
  (`roadmap_id=None`)은 영향 없음. 테스트 4건 추가, 268개+골든 6개 전부 통과.
- **[feat] #223 — "효원(균형·창의)교양" 세부영역 필터 단일선택→다중선택.**
  PR #221의 필터가 단일선택이라 "균형교양 최소 2개 영역에서 2과목" 같은 요건은
  영역 하나로만 좁히면 후보를 못 찾았다. `GET /courses/offerings`의
  `general_education_area`를 `list[str]`+`.in_()`로, 프론트 칩을 토글식 다중선택으로.
  "필터가 아예 안 먹는다"는 원 신고는 로컬 dev+실 Supabase로 재현 안 됨(단일선택
  자체는 정상 동작) — 원인은 특정 못했지만 다중선택 전환으로 그 혼동 경로 자체는
  없어짐. Playwright로 실계정 시나리오 재현: 357→46(사상과역사)→115(+사회와문화,
  정확히 46+69 합집합)→69(사상과역사 해제).
- **[docs] #224 — 교양 영역 이수요건 조사 21→34개 학과.** §7.3 2단계 착수 전 원문
  조사 계속. `raw_data/manual_staging/01_graduation_requirements/by_department/`에
  이미 142개 학과 원문이 있다는 걸 재확인(설계 문서의 "59개"는 갱신 안 된 옛 숫자)
  — 36개 학과를 3배치로 조사해 13개 추가 확인, 새 변형 패턴 7개 발견(균형 5영역
  모집단, 전원수 이수, 영역배제, 지정과목, 자기학과 수강금지, 원문 인쇄오류 등).
  화학과 PDF "스캔이미지" 오기도 정정. 조사만, 스키마/엔진 변경 없음.

## 2026-08-24 (blackest21) — 시간표 작성 화면 "효원(균형·창의)교양" 갈래에 세부영역 필터 추가

사용자 관찰: 시간표 작성에서 "효원(균형·창의)교양" 갈래를 고르면 357건(2026-2학기)이
필터 없이 그대로 쏟아져서 원하는 영역(사상과역사 등)을 찾기 어려웠다.

- **[api]** `GET /courses/offerings`에 `general_education_area` 쿼리 파라미터
  추가 — `courses.general_education_area`(PR #199/#211에서 이미 채워둔 컬럼)와
  정확히 일치하는 것만 남긴다. 응답(`OfferingSearchResult`)에도 그 값을
  실어 보낸다(어느 영역 과목인지 줄마다 알 수 있게, `major_name` 노출과 같은 이유).
- **[frontend]** 시간표 작성 화면에서 "효원(균형·창의)교양" 갈래를 고르면 전공
  갈래의 이수구분 칩(전공기초/필수/선택)과 같은 자리에 세부영역 칩 9개(사상과역사·
  사회와문화·문학과예술·과학과기술·세계와 소통·효원브릿지·건강과레포츠·융합과
  창의·인성과 사회봉사)가 뜬다. 안 고르면 기존과 동일하게 전체가 나온다(옵트인).
- **검증**: `pytest` 신규 2건 포함 626 passed, `npm run build` 통과. 실제 Supabase로
  직접 확인 — 필터 없음 357건 → "사상과역사" 46건, "세계와 소통" 66건으로 정확히
  좁혀짐(영역명 공백 표기 "세계와 소통"까지 실데이터로 재확인).

## 2026-08-24 (blackest21) — 시간표 챗 골든 eval 3건 추가 — 복수전공 시간충돌·재수강·졸업위험 재분류

PR #209 Q&A에 명시적으로 남겨뒀던 후속 과제 3개(복수전공 시간표 충돌 처리, 재수강
반영, 졸업위험 경고를 시간표 챗 맥락에서)를 채웠다. 로드맵 챗엔 대응 케이스가 이미
있지만(09 dual, 22 retake, 13 critical_missing), 시간표 챗은 도구 스키마가 아예
달라서(`build_timetable`엔 `propose_change`의 `is_retake` 같은 우회 플래그가 없음)
같은 로직이라도 실제로 지켜지는지 따로 검증해야 의미가 있다.

- **[eval] 케이스 30 (복수전공 시간충돌)** — 주전공(정컴) 필수와 복수전공(수학과)
  필수 분반을 정확히 같은 시간대로 겹치게 설계. 시간 충돌 회피가 프로그램 구분과
  무관하게(주전공이든 복수전공이든) 동작하는지 검증 — 지금까지 유일한 dual 케이스
  (09)는 로드맵 챗이라 이 경로가 실측된 적이 없었다. 3/3 라이브 통과.
- **[eval] 케이스 31 (재수강은 조언만)** — `build_timetable`엔 재수강 우회 플래그가
  없다는 걸 코드로 확인(`grep`으로 `is_retake` 0건). 즉 재수강 후보라도 이미
  이수한 과목은 무조건 필터에서 빠지는 게 설계 의도(코드 주석: "재수강은 별도
  신청 절차")다. 이 "조언만 하고 강제 편성 안 함"을 처음으로 검증. 3/3 라이브 통과.
  부수 관찰: LLM이 이수 완료 과목을 "이번 학기 미개설"이라고 살짝 부정확하게
  설명하는 경우가 있었다(실제로는 "이미 이수"라 빠진 것) — 기능적으로는 항상
  올바르게 제외됐으니 블로커는 아니지만 문구 개선 후보로 남김.
- **[eval] 케이스 32 (졸업위험 재분류)** — `_critical_missing_split`(카탈로그
  권장학기만 보고 "미개설"로 오판하지 않고 실제 `course_offerings`로 재교정하는
  로직, 공학작문및발표 실사고를 고친 코드)을 처음으로 검증. **정직하게 기록**:
  10회 라이브 실행 중 8/10 통과. 실패 2건을 `tool_calls` 직접 캡처로 근본원인까지
  추적했다 — 백엔드 재분류 로직 자체는 10/10 항상 정확했고(별도 스크립트로 4회
  재현, 전부 정확), 실패는 `list_offered_courses`를 필터 없이 넓게 호출해 0건이
  나왔을 때 LLM이 가끔 재검색 없이 바로 사용자에게 되묻고 끝내버리는 tool-use
  전략의 비결정성 때문이었다(나머지 대다수는 같은 0건 상황에서도 과목명으로 좁혀
  재검색해서 항상 찾아냄). **1회 통과로 "됐다"고 안 쓰고, 실패도 나오는 대로
  받아들여 원인까지 판 뒤 케이스는 그대로 남겼다** — 검증 대상(재분류 로직) 자체는
  결함이 없다는 게 확인됐고, 남은 20%는 "빈 검색 결과 시 재검색 유도" 프롬프트
  개선 후보로 케이스 docstring에 남겨뒀다.
- 시간표 챗 골든 케이스 7 → 10, 전체 골든셋 29 → 32. dry-run 32/32, `pytest` 624
  passed 모두 재확인.

## 2026-08-24 (blackest21) — 인시던트: PR #215가 Supabase 마이그레이션 없이 머지돼 배포본 회원가입이 500으로 깨짐 → 즉시 복구

**타임라인**: PR #215(개인정보 동의 체크박스, `users.privacy_consent`/`privacy_consent_at`
컬럼 추가 포함)를 "Supabase 마이그레이션 먼저 적용 후 머지"라고 PR 본문/CHANGELOG에
명시해서 머지 보류 중이었다. 문서만 담은 후속 PR #216을 리뷰하는 동안, 팀원(D0won)이
#216뿐 아니라 **#215까지 같이 머지**했다. Railway가 새 코드로 재배포되면서, 실제
배포본이 `POST /auth/signup`에서 `users.privacy_consent`를 참조하는데 Supabase엔
그 컬럼이 없어 **회원가입이 500으로 깨졌다**(직접 `curl`로 재현 확인).

**원인 조사 중 추가로 발견한 것**: Supabase의 `alembic_version`이 `7b3e9c1a4f20`이었는데,
그 형제 브랜치 `5ae96e0e4d56`(`courses.general_education_area` 추가)는 **컬럼 자체는
이미 존재하는데 alembic 기록엔 없었다** — 이전 세션 어느 시점에 다른 경로로 반영되고
alembic 기록만 안 됐던 것으로 보인다. 그래서 단순 `alembic upgrade head`는 "컬럼이
이미 있다"는 에러로 실패했을 것이다.

**복구 절차(사용자 확인 후 실행, 전부 이 세션에서 직접 수행)**:
1. `alembic stamp 5ae96e0e4d56` — DDL 실행 없이 이미 적용된 사실만 기록(안전).
2. `alembic upgrade head` — 병합점(빈 마이그레이션) 통과 후 `privacy_consent`/
   `privacy_consent_at` 컬럼 실제 추가.
3. `curl`로 실제 `/auth/signup` 재호출 → 201 성공 확인, 테스트 계정은 즉시 삭제.
4. `pytest tests/`를 `.env`(Supabase) 기준으로 재실행 → **624 passed**, 앞서 실패하던
   `RateLimitTest` 2건도 통과.

**장애 시간**: PR #215 머지 시점부터 위 복구 완료까지. 정확한 분 단위 파악은 안 했지만
같은 세션 안에서 발견 즉시 조치해 길지 않았다.

**재발 방지 메모**: "머지 전 마이그레이션 먼저"를 PR 본문에 적어두는 것만으로는
막지 못했다 — 리뷰·머지 권한이 있는 다른 팀원이 그 문구를 놓치거나 별개로 판단할 수
있다. 다음엔 **머지 자체를 막는 장치**(예: 별도 승인 라벨, 또는 마이그레이션이 실제
Supabase에 반영됐는지 확인하는 CI 체크)를 고려할 것 — 지금은 사람이 PR 설명을 읽는
것에만 의존했다.
...
- **F-01**: "SW/AI융합트랙"이라는 또 다른 옛 용어 변형이 남아있었다(오늘 이미 두 번
  고친 "SW·AI융합트랙"/"SW융합"과 별개 변형) → "AI융합트랙"으로 통일
  (`tracks.py:24`의 `TRACK_CERTIFICATION_TYPE` 확인).
- **F-08**: 1.3절은 오늘 이미 "미도입 — 최우선 처리 대상"으로 정정했는데 F-08은
  여전히 "배포 시점에 확정"이라는 옛 미래형 문구를 써서 같은 사실을 서로 다르게
  서술하고 있었다 → 1.3과 같은 표현으로 통일.
- F-02(선수과목·학점상한 검증)·F-03("담기" 용법)·F-09(파기 도구 미활성화)는
  실측 결과 이미 정확해서 안 건드림.
- **3.4 디렉토리 구조 누락 5곳**: `ai/`가 `llm/`(Langfuse 콜백·마스킹)을 빼고 RAG만
  적혀 있었음, `domains/`에 `content/`(연결 안 된 스캐폴딩)가 빠짐, `core/`에
  `ratelimit.py`/`scheduler.py`가 빠짐, `frontend/src/`에 `data/`가 빠짐, 최상위에
  `infra/`·`.github/`가 아예 없었음(제출용 핵심 구조 요약인데 CI/로컬 검증 체계가
  안 보임) → 전부 추가.
...
어떻게 작동하냐")로 발각. 배포된 프론트(`https://planu-pnu.netlify.app`)의 실제 JS
번들을 받아 확인하니 `VITE_API_BASE_URL`이 이미
`https://pnuai-a-03-team-u-production.up.railway.app`로 빌드돼 있었다. 그 주소에
직접 요청해보니 `/health` → `{"status":"ok"}`, `/openapi.json`의 `title` →
`"Plan U Backend"` — **진짜 우리 백엔드가 이미 살아서 실사용자 트래픽을 받고 있다.**

이 세션 앞부분에서 나 스스로도 README의 "Railway(예정)" 문구를 그대로 믿고 Langfuse
작업(PR #212)을 "Railway 미배포라 다들 로컬로 돌린다"는 전제로 진행했다 — **틀린
전제였다.** 게다가 CHANGELOG 자체에 이미 단서가 있었다: 2026-08-20 근처 항목(비밀번호
재설정 PR)에 "남은 것(코드 아님): 배포 백엔드(Railway)에 SMTP_*와
PASSWORD_RESET_URL_BASE가 없어서 배포본에서는 여전히 메일이 안 나간다"는 문장이 이미
있었다 — Railway가 그때부터 이미 배포돼 있었다는 뜻인데, 그 이후 아무도(나 포함) README를
안 고쳤고 나는 그 CHANGELOG 항목도 놓쳤다.

**고친 것(README, 문서만)**:
- 2.1 시스템 구성도 mermaid: "Netlify(예정)"/"Railway(예정)" → "Netlify"/"Railway"
- 2.2 기술 스택 표: "배포 (예정)" → "배포"
- 1.3 개인정보 보호 원칙: "저장·전송 보안(적용 예정)... 로컬 환경에서만 구동한다" →
  이미 배포·HTTPS 적용된 상태로 정정. **단, "수집 최소화 및 동의"·"개인정보처리방침·LLM
  처리위탁 고지" 두 항목은 원래 "배포 전에 도입한다"고 적혀 있었는데, 이미 배포된 상태에서도
  빠져 있는 것이므로 "적용 예정"이 아니라 "미도입 — 최우선 처리 대상"으로 정정했다.
  이건 문서 정정이 아니라 실제로 남은 작업이다.**
- 4절 앞에 배포된 프론트/백엔드 실제 URL 추가.

**후속 리뷰가 잡은 것**: 위에서 "동의 절차·처리방침 문구가 전혀 없다(grep 0건)"이라고
썼는데 부정확했다 — `OnboardingPage.tsx`에 "✓ 개인정보 수집 · 이용 동의 · 성적표 원본은
저장하지 않습니다"라는 문구 자체는 이미 있다(`grep -rn "동의" frontend/src` → 1건).
다만 이게 체크박스 같은 실제 옵트인 컨트롤이 아니라 그냥 표시 텍스트라서, 사용자가
실제로 동의했는지와 무관하게 항상 "✓"로 보인다 — **동의를 안 받은 것보다 오히려 더
문제("받은 것처럼 보이지만 실제로는 안 받음")일 수 있다.** README 본문(1.3)은 이 정확한
내용으로 다시 정정했다("grep 0건"이 아니라 "표시 문구만 있고 실제 옵트인 절차 없음").
"처리방침"이라는 단어 자체는 실제로 0건이 맞았다(`grep -rn "처리방침" frontend/src`).

**확인 안 하고 넘어간 것 (다음에 볼 것)**:
- 2026-08-20 CHANGELOG 항목이 지적한 "배포 백엔드에 SMTP_*/PASSWORD_RESET_URL_BASE
  없음" 문제가 그 이후 해결됐는지 확인 안 함(Railway 대시보드 접근 권한이 없어 직접
  조회 불가) — 아직도 안 됐다면 배포본에서 비밀번호 재설정 메일이 안 나가는 상태일 수
  있다.
- `LANGFUSE_*`(PR #212)도 로컬 `.env`/기본값만 바꿨지 실제 Railway 서비스 환경변수는
  아직 안 건드림 — 사용자에게 Railway Variables에 직접 넣을 값 4개를 전달함(별도
  기록 안 함, 로컬 `.env`와 동일). 넣기 전까지는 배포본의 실사용자 trace가 Langfuse에
  전혀 안 쌓이고 있었을 가능성이 높다.
- 위 두 가지 다 Railway 대시보드에 직접 접근해야 확인 가능 — 이 세션에서는 CLI/토큰이
  없어서 못 함.

## 2026-08-24 (blackest21) — README 3.1/3.2 다이어그램이 로드맵→시간표를 순차 파이프라인처럼 잘못 그렸던 것 수정

"로드맵과 시간표가 왜 연결돼 있냐, 독립이어야 하는 거 아니냐"는 지적으로 시작. PR #210에서
그린 3.2 mermaid가 `RM4 --> APPROVE1 --> TT`로 화살표를 이어서, 마치 로드맵 에이전트 제안을
사용자가 승인해야만 시간표 에이전트로 넘어가는 것처럼 그려놨었다. 3.1(전체시스템 흐름도)도
같은 문제로 "성장 로드맵 → 다음 학기 시간표"를 화살표 하나로 이어놨었다.

독립 세션 2개(코드 실측, 서로 다른 파일/관점)로 각각 확인:

- **로드맵↔시간표 관계**: `timetable_chat.py` 파일 docstring이 이미 명시하고 있었다 —
  "로드맵과 독립적으로 동작한다... `roadmap_id`가 필요 없다." `get_roadmap_hint`는 사용자가
  로드맵을 언급할 때만 선택적으로 참고하는 힌트일 뿐 필수 입력이 아니다(`timetable_chat.py:156,
  2096-2122`). 반대 방향(`roadmap_chat.py`)은 시간표 쪽 데이터(`CoursePlan`/`CoursePlanItem`)를
  import조차 안 한다. API 라우터에도 순서를 강제하는 검증 코드가 없고, 프론트 라우팅
  (`AppRouter.tsx:48-49`)도 `/roadmap`·`/timetable`이 가드 없는 형제 라우트다. **유일한 실제
  데이터 연결**은 시간표 화면의 "로드맵에 반영" 버튼(`POST /{roadmap_id}/timetable/apply`,
  `roadmaps.py:420-539`) — 확정한 분반을 로드맵 항목으로 직접 `db.add`해서 즉시 커밋하는
  단방향 경로이고, 로드맵 쪽 `PendingRoadmapChange` 승인 절차와는 무관하다.
- **3.1 전체 흐름도**: "학사 정보 동기화"가 실제로는 학적·이수내역·지도교수뿐 아니라
  비교과 활동·자격증·어학성적까지 같은 크롤링 파이프라인으로 가져오는데(`portal_sync.py:
  37-47,270-282,332-346`) 3.1엔 이게 빠져 있었다. 그리고 온보딩(회원가입→동기화) 이후
  "졸업요건 분석/성장 로드맵/시간표 작성" 3개 화면은 순서 강제가 전혀 없다 — 동기화 전에
  들어가도 막지 않고 빈 상태만 보여준다(`RoadmapPage.tsx:2081,2171,2186`).
- **부수 발견**: README가 여전히 "SW융합"이라는 옛 용어를 썼다(코드는 `tracks.py`에서
  "AI융합트랙"으로 통일된 지 오래). "비밀번호 찾기는 추후 개발 예정"이라는 문구도 실제로는
  이미 완전히 구현돼 있었다(`auth.py:300-360`, 토큰 발급·만료·1회용 무효화까지) — README가
  안 따라간 부분.

**고친 것**: 3.1은 SIGNUP→SYNC까지만 순차로 유지하고 그 뒤 RULE/RM/TT를 SYNC에서 병렬로
갈라지게 재구성, TT→RM "로드맵에 반영"을 점선 단방향 화살표로 별도 표시. 3.2는 RULE에서
RM/TT 서브그래프가 동시에 갈라지고 각자 독립적으로 APPROVE→RESULT에 합류하도록
재구성(번호도 ④/④', ⑤/⑤'로 병렬임을 표시), "로드맵에 반영" 점선 화살표 추가.
"SW융합"→"AI융합트랙", 비밀번호 찾기 문구 정정(README 219행, 321행 SMTP 설명도 같이).

**리뷰에서 잡힌 것**: 초안에서 이 버튼을 "담기"라고 잘못 불렀다 — "담기"는 검색한 강좌를
열려 있는 시간표 초안에 넣고 빼는 별개 동작(`TimetablePage.tsx:1174-1176` 주석)이고,
로드맵에 실제로 반영하는 버튼 텍스트는 `TimetablePage.tsx:1186` 기준 "로드맵에 반영"이
맞다. 독립 리뷰가 이 5곳(README 4곳 + 이 CHANGELOG 항목)을 잡아내서 병합 전 정정했다.
같은 오류가 이번 PR이 안 건드린 3.2 "화면별 상세 · 시간표 작성"(README 247행, 원래부터
있던 문장)에도 있어서 김에 같이 고쳤다.

관련 문서: 이 항목이 CHANGELOG 갱신 전부 — 별도 feature 문서 변경 없음(README 자체가 대상).
...
확인 결과 "항상 4년 전체를 조용히 계획"이 아니라 **"로드맵 만들어달라고 하면
'졸업까지 만들까요?'라고 먼저 물어봐주고, 교양은 매 학기 바뀌니 정확한 강의명보다
세부영역 위주로 채워달라"**는 의미였다.

- **[feat] 범위 미지정 요청은 제안 전에 먼저 확인.** "로드맵 짜줘"/"성장 로드맵
  만들어줘"처럼 다음 학기 하나인지 졸업까지 전체인지 안 밝힌 요청에, 예전엔 그냥
  다음 학기 하나만 조용히 골라 진행했다(전체를 원한 사용자는 매번 "이어서 해달라"고
  다시 요청해야 했다). 새 조건부 규칙 `unscoped_build_request`를 추가해
  `propose_change`/`propose_term_plan`을 먼저 부르지 말고 "졸업까지 전체 학기를
  계획할까요, 다음 학기만 볼까요?"로 되묻게 했다. 이미 범위를 짚은 요청
  ("다음 학기 뭐 들을까", "3학년 2학기 계획해줘")이나 이미 "졸업까지"라고 못박은
  요청(`full_horizon_request`, 그쪽은 반대로 "먼저 되묻지 마라"가 규칙)에는 안 걸린다.
- **[feat] 먼 미래 학기의 교양선택은 정확한 과목명이 아니라 세부영역 예시로.**
  균형/창의교양은 학기마다 실제 개설 과목이 바뀐다(이번 세션에서 직접 확인 —
  같은 영역이라도 학기별로 다른 과목이 열림). `full_horizon_request`(여러 학기
  한 번에 계획) 규칙에 지침 추가: 다음 배치 가능 학기가 아닌 그 이후 학기의
  교양선택은 `search_courses(liberal_area=X)`로 지금 검색되는 후보 하나를 예시로
  넣고 reason에 "실제 개설은 학기마다 바뀌니 수강신청 시점에 재확인 필요"를
  명시하게 했다. 다음 배치 가능 학기(곧 실제로 신청할 학기)는 예외 — 평소대로
  확정적으로 추천.
- 회귀 테스트: `_looks_like_unscoped_build_request` 단위 테스트(트리거/비트리거
  케이스), `_build_system_prompt` 규칙 배선 테스트 — `test_roadmap_chat.py`.
  eval 골든셋에 케이스 27(`27-unscoped-roadmap-request`) 신규 추가 —
  `tool_not_called`(propose_change/propose_term_plan 둘 다) + `llm_judge`(범위를
  실제로 되묻는지).
- **[fix, 독립 리뷰가 잡음] 프롬프트 카피만으로는 신뢰성이 부족했다.** 처음엔
  `--live --case 27` 1회 실행이 우연히 통과해서(2026-08-23) "확인됨"으로 적었는데,
  PR 리뷰 세션이 같은 케이스를 3번 재실행하니 **3/3 전부 실패**했다 — 다른 조건부
  규칙(`critical_missing`, 이 페르소나는 졸업 위험 경고도 같이 뜬다)이 동시에 붙으면
  "일단 파악한 걸로 다음 학기라도 만들어서 보여주자"는 쪽으로 도구를 불러버렸다.
  규칙 문구를 훨씬 강하게 고쳤다 — "이 턴의 propose 계열 도구 호출은 0회",
  "다른 규칙이 같이 붙어도 이 규칙이 이긴다"를 명시적으로 못박고, 위험 정보는
  finish_response 텍스트로만 요약하라고 분리했다. 재검증: **`--live --case 27`을
  5회 반복해 5/5 PASS**(모델 확률성 감안 최소 3회 이상 재현 확인 원칙 — 1회
  통과로 "됐다"고 적은 게 이번 실수였다, 앞으로는 최소 3회 반복 확인).
- **알려진 한계**: 범위 판정은 여전히 문자열 마커 기반 휴리스틱이다. "작년
  1학기엔 이미 다 들었는데 로드맵 짜줘"처럼 과거 학기를 언급하면서 범위를 안
  밝힌 요청은 `_TERM_SCOPED_MARKERS`가 "1학기"라는 단어에 걸려 이미 범위가
  정해진 것으로 오판하고, 결과적으로 아무 조건부 규칙도 안 붙어 기존 기본
  동작(다음 학기만 조용히 진행)으로 빠진다. 드문 문장 패턴이라 이번엔 안
  고쳤다 — 다음에 이 휴리스틱을 또 건드릴 일이 있으면 같이 볼 것.
- 검증: `pytest` 614 passed, 골든테스트 9/9, eval dry-run 27/27,
  eval `--live --case 27` 1/1 PASS.

## 2026-08-23 (blackest21) — 1년치(봄·가을·여름) 카탈로그 교차확인 + 신설 융합전공 13개 정식 등록

"가을 학기 편람만 봤는데, 봄·여름도 확인하면 전체 전공구분을 확실히 알 수 있지 않냐"는
지적으로 시작. 실제로 그랬다 — 학기별로 **서로 다른 과목이** 열려서, 가을만 봐서는
전체 그림의 절반도 못 봤다.

- **[note] 크롤러 스코프 재확인.** `onestop_course_catalog.py`는 `subject_category_code
  1~5`(전공,교직/전공기초/효원균형창의교양/효원핵심교양/일반선택 — 전부 학부 수강신청
  카테고리)만 돈다. 대학원 태그가 붙은 행은 4282건 중 8건뿐이고 그나마도 우리
  TARGET_CATEGORIES 밖이라 애초에 안 걸린다 — "358건 갭이 전부 학부 과정"이라는
  전제가 맞았다.
- **[feat] 2026-1(봄)·2026-summer(여름) 카탈로그 갭도 반영.** 로컬에 이미 크롤링돼
  있던 CSV(`raw_data/crawled_data/onestop_course_catalog/2026_1`,`2026_summer`)에
  기존 스크립트(코드 변경 없이 `--csv`만 바꿔 재실행)를 그대로 돌렸다:
  - 균형/창의교양: 봄 109건 + 여름 1건
  - 전공/일반선택: 봄 169건(학과매칭 137 + 행정부서 32) + 여름 18건(전부 행정부서)
  - 겨울(2026-winter)은 크롤 결과가 0건 — 아직 개설 공지 자체가 안 났다(정상).
  - **여름 카탈로그에서 "SW문제해결프로젝트"(AI융합 공통교과목, SF1101032)를
    발견** — 가을엔 없어서 "원문 자체에 없다"고 잘못 결론 낼 뻔했는데, 계절학기
    전용 개설이었다. `인공지능기반창업`만 봄/가을/여름 전부에 없어 진짜 미확인으로
    남음(3학기 교차확인으로 근거 강화).
- **[feat] 신설 융합전공 13개(2026 학부내 트랙, 이전 세션 "99건 백로그") 정식 등록.**
  AIS 위젯 프록시(`bkorea.pusan.ac.kr`, `reference_pnu_curriculum_research.md`
  방법)로 `/ais/getDeptInfoDeptList` + `/ais/UNIV/{collegeCd}/getDeptInfoMajorList`
  직접 조회 — 13개 전공이 실제로 어느 단과대/학부 소속인지 정확히 확인:
  - 독립 학과로 등록(7개): 미래모빌리티전공·해양미래산업전공·AI혁신전공·반도체융합전공
    (공과대학), DX융합전공(학부대학), 그린바이오융합전공(생명자원과학대학),
    통합사회전공(사범대학)
  - 기존 학부 산하 전공으로 등록(6개): 기계공학부-기계시스템설계전공,
    첨단융합학부-클린에너지전공/이차전지융합전공/지식재산융합전공,
    의생명융합공학부-첨단바이오공학전공, 국제학부-글로벌스터디즈 프로그램
  - 상위 단과대(공과대학/학부대학/정보의생명공학대학/생명자원과학대학/사범대학/
    경제통상대학)와 부모 학부(기계공학부/첨단융합학부/의생명융합공학부/국제학부)는
    **전부 이미 DB에 있어 새로 안 만들었다** — `seeds/school_hierarchy_mapping.csv`에
    13행만 추가(`split_rule=ais`, 기계공학부 세부전공 5개가 이미 쓰던 것과 같은
    패턴), `scripts/seed_school_hierarchy.py`(기존 스크립트, 안 바꿈)로 반영:
    `departments` 122→129(+7), `majors` 62→68(+6).
  - `academic_programs_2026_master`(2026학과전공분류자료집)에는 13개 다 미등재 —
    공식 분류 워크북이 2026년 신설 학부내 트랙까지는 못 따라간 것으로 보인다.
    T코드(T00000012662~012674)로 임시 학사프로그램코드 부여, 정식 커리큘럼 원문
    수집(`_collection_targets_index.csv` 편입)은 여전히 별도 작업.
  - 등록 직후 카탈로그 스크립트를 3학기(봄/가을/여름) 전부에 재실행(코드 변경
    없음) → 이제 이름이 매칭돼 205건 추가 반영.
- **최종 상태**: `courses` 6950 → **7451**(+296 봄·여름 갭 + 205 신설전공 매칭),
  `departments` 122→129, `majors` 62→68. 남은 스킵은 39건(한의학과 32 + 극소수
  잡음 7건: 미래자동차융합전공/환경공학전공/임상시험코디네이터/공학교육혁신센터/
  학생과/교양교육원 각 1~2건) — 전부 `raw_data/reports/major_course_gap_2026_*_round3.csv`.
- 검증: `pytest` 612 passed, 골든테스트 9/9 — 무변경 확인.

## 2026-08-23 (blackest21) — 전공 카탈로그 갭 252건 후속: 행정부서 개설 133건 반영 + 남은 119건 사유 확정

PR #203이 남긴 252건 스킵 목록을 재검토해 정확히 세 부류로 나눴다(합이 252와 정확히
일치). "행정부서/법학과가 왜 학과가 아니냐"는 질문으로 시작해서, 이 과목들이 애초에
특정 학과 소속이 아니라 전교생 대상으로 열리는 것임을 확인했다.

- **[feat] 133건 반영(department_id=NULL).** 학사과/현장실습지원센터/창업교육센터/
  취업전략과/언어교육원 등 행정·지원부서 + 법학과가 개설한 과목 133건 — 전부
  이수구분이 "일반선택"뿐이었다(전공필수/선택 섞인 게 하나도 없음). 내용은
  교양스럽다(창의적문제해결력/진로탐색과생애설계/장기현장실습/고급한국어 등)
  — 그런데 PNU 원문 이수구분 자체가 "교양선택"이 아니라 "일반선택"이라 재분류하지
  않고 원문 그대로 뒀다. 균형/창의교양과 같은 방식(department_id=NULL → 전교생
  검색 노출)으로 채웠다. `scripts/seed_major_courses_from_offerings.py`에
  `INSTITUTION_WIDE_OFFERING_UNITS` 상수 추가.
  - 법학과가 이 그룹에 들어간 이유: `academic_programs_seed_2026_active_bachelor.csv`
    (152개 활성 학사과정 전체 목록) 대조 결과 **법학과는 학부 자체가 없다**(로스쿨
    체제 전환, 대학원 과정만 존재). 그런데 이 18건이 전부 전공필수/선택이 아니라
    일반선택이라, "법학과 전공생만 듣는 과목"이 아니라 개방형 교양성 과목(생활과
    법률류)으로 보고 같이 채웠다.
- **[note] 한의학과(20건) — 확정된 공백, 액션 불필요.** 같은 152개 목록 대조 결과
  한의학과도 학부 없음(한의학전문대학원/일반대학원 대학원 과정만 존재) — 이쪽은
  전공필수/선택 과목이라(내용상 진짜 전공 커리큘럼) 재분류 대상이 아니라 그냥
  대상 밖.
- **[note] 신설 융합전공 + 글로벌스터디즈 프로그램(99건) — 여전히 백로그.**
  미래모빌리티전공/클린에너지전공/이차전지융합전공 등 12개 — `departments`/
  `majors`는 물론 `_collection_targets_index.csv`(151개 공식 수집 대상)와
  `academic_programs_2026_master`(2026 학과 마스터 전수)에도 이름이 없다. 정식
  학사 프로그램 등록이 먼저 필요해 이번에도 스킵. 최신 스킵 목록:
  `raw_data/reports/major_course_gap_skipped_2026-08-23_round2.csv`(119건).
- **[note] AI융합교육원 공통교과목 실데이터도 같이 점검(테스트 아님, 앞 항목에
  기록)** — 카탈로그 8/10, 실개설 3개, 2개는 원문 자체에 없음. 이번 반영과는
  무관(그 두 과목은 이 133건에 안 걸림).
- Supabase 반영: `courses` 6817 → 6950(+133, 전부 department_id=NULL·일반선택),
  이어서 `import_course_offerings.py` 재실행으로 `course_offerings`도 채움.
- 검증: `pytest` 612 passed, 골든테스트 9/9(무변경 확인).

## 2026-08-23 (blackest21) — 골든테스트 다양화(TC07~09) + AI융합 공통교과목 실데이터 점검

골든테스트(TC01~06)가 전부 컴퓨터공학과 위주라는 지적 — AI융합트랙 대상 학과, 비CS
학과 전 카테고리, 부전공(minor) 시나리오가 하나도 없었다.

- **[test] TC07_AI_TRACK_INTERDISCIPLINARY** — AI융합트랙 대상 14개 학과 중 하나
  (산업공학과, 실 Supabase 조회 기준 실제 전공명·학점 구성 그대로)가 트랙 요건을
  채우면 `is_ai_track=True`로 판정되는지 검증. `ProgramProgress.is_ai_track`(오늘
  이슈 #202 수정에서 추가한 필드)이 골든셋 레벨에서도 실제로 맞물리는지 처음 확인.
- **[test] TC08_NON_CS_PRIMARY_FULL_BREAKDOWN** — 간호학과(전공필수 비중이 아주
  큰 실제 커리큘럼) 주전공, 6개 카테고리 전부 실 데이터 그대로.
- **[test] TC09_MINOR_PROGRAM** — 골든셋에 지금까지 복수전공만 있고 부전공이
  아예 없었다. 경영학과 부전공으로 추가 — 실 데이터에서도 minor 요건 행 다수가
  카테고리 세분 없이 총학점만 있다는 실제 모양 그대로 재현(억지로 카테고리를
  지어내지 않음).
- `run_golden_tests.py`의 `_check_program_result`에 `is_ai_track` 대조 추가.
- **[note] AI융합 공통교과목 10개 실데이터 점검(2026-2학기, 실 Supabase 조회)**:
  카탈로그엔 8/10 존재, 실제 개설은 3개(AI이해를위한파이썬기초/데이터리터러시의이해/
  데이터 마이닝)뿐. `인공지능기반창업`/`SW문제해결프로젝트`는 수강편람 원문 자체에
  없음(매칭 로직 문제가 아니라 진짜 미개설) — PR #199 본문이 이미 기록한 상태와
  동일, PR #199/#203의 카탈로그 갭 보정 이후에도 변화 없음. `AI활용디지털전환`은
  구명칭(`창의적 프로그래밍`)으로 정상 매칭 확인.
- 검증: `python tests/run_golden_tests.py` 9/9 PASS, `pytest` 612 passed(무변경 확인).
...
- **[feat] 목표는 "학기를 꽉 채우기"가 아니라 "졸업요건 충족"이다.** 되돌림 게이트의
  ...
  - **[fix] 교양 후보 검색과 요건 집계가 `기초교양`에서 정반대였다.** 검색
    (`curriculum_retriever._CATEGORY_ALIASES`)은 `기초교양`을 `교양필수`로 돌려주는데
    집계는 `교양선택`으로 셌다. LLM이 교양필수를 채우려고 검색 → 기초교양을 받아 제안
    → 교양필수 잔여는 그대로 → 게이트가 다시 교양필수를 요구하는 **루프**가 난다.
    운영 DB 교양 58건 중 기초교양이 22건으로 최대 버킷이라 가설이 아니다. 원문
    (교양교육원 수강지도 지침: 2005~2021 "기초교양과목은 **교양선택** 이수학점에 포함",
    2026 "효원균형·창의교양 이수학점에 포함")상 집계 쪽이 맞다. 두 벌을 손으로 유지하다
    어긋난 것이라, 검색 alias를 `COURSE_CATEGORY_TO_REQUIREMENT`에서 **역인덱싱**해
    단일 출처로 만들었다. 옛 매핑을 고정하던 테스트도 근거와 함께 바로잡았다.
  - **[fix] 매핑의 "세 번째 사본"이 프롬프트에 남아 있었다.** 코드 두 벌은 단일 출처로
    합쳐놓고, 정작 LLM에게 나가는 문구 3곳은 `'교양필수' → 효원핵심교양+기초교양`이라는
    옛 매핑을 그대로 가르치고 있었다. 판정은 엔진이 하니 오판은 없지만, LLM이 최대
    버킷인 기초교양을 교양필수 후보로 믿고 찾다 놓친다. 스스로 지적한 "단일 출처라
    써놓고 두 벌을 남겨뒀다"를 한 번 더 반복한 셈이라 같이 고쳤다.
  - **[fix] 되돌림 문구가 "아직 0학점이 미배정이다()"였다.** 총학점만 남았거나 요건을
    모를 때도 게이트가 걸리는데, 문구는 이수구분 잔여만 읽었다. 되돌림은 finish를 막는
    가장 강한 신호인데 도구 자신의 `next_action`과 반대말을 하고 있었다.
  - **[fix] 총요구학점을 모르면 "충족"을 주장하지 않는다.** 이수구분 잔여가 0이어도
    총학점 축이 통째로 빠진 채 판정하게 된다.
