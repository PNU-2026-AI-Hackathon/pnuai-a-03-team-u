...
## 2026-07-06 (hyunwoocho) #14

- 실내환경디자인학과 학년/학기 정보 35건 복구 + 카탈로그 잔여 데이터 정리(3,544건) — 부수적으로 새 gap 발견
  - 사용자가 `hid/10663` 페이지를 다시 알려줘서 재확인하다가, 학과 게시판(hid/10664)의 2016~2018학년도
    구 교육과정표 PDF는 L7 WAF에 안 걸린다는 걸 발견(최신 PDF만 차단). 과목명/코드 기준으로 대조해서
    기존 40건 중 35건의 recommended_year/semester 복구·confirm. 5건(신설로 추정)만 미상으로 남음
  - 사용자 지적("카탈로그 추정은 학과에서 이미 찾았으면 필요없지 않냐")으로 실제 원문이 충분히 확보된
    학과의 `department_courses_from_catalog` 잔여 데이터 정리. 기준: `requirement_course_supplemental`
    존재 또는 자동파싱 15건 이상인 학과만 선별해서 카탈로그 데이터 drop(무작정 전부 지우지 않음 — 기준
    미달 학과는 카탈로그가 유일한 정보라 그대로 유지)
  - 결과: 80개 학과 3,544건 drop, `requirement_courses` 15,546 -> 12,002건. 드롭 후 0건 된 학과 없음
    확인(기존 확정된 7개 학과와 동일). 골든테스트 8개 재통과
  - **부수 발견**: 이 정리 과정에서 "카탈로그 추정뿐이고 실제 원문 커버리지가 사실상 없는" 학과가 기존에
    알던 3개(경제학부/스마트가전공학과/치의예과)보다 훨씬 많다는 게 드러남 — 총 26개(원문 완전 0건 4개:
    일반사회교육과 신규 발견 포함, 산발적 매칭 1~14건뿐인 22개: 서양화전공/지리교육과/고분자공학과/
    자유전공학부/첨단융합학부/미술학과/지질환경과학과/한국화전공/생명과학과/도시공학과/
    첨단모빌리티자율전공/지구과학교육과/의생명융합공학부/첨단소재자율전공/수학과/역사교육과/조소전공/
    IT응용공학과/국어국문학과/불어교육과/해양학과/글로벌자유전공학부). 다음 세션에 원문 재탐색 필요 —
    상세는 `local.md`(개인 노트) 참고
