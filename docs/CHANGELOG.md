...
- **[perf/security] 메일 발송을 응답 뒤로 미룬다.** SMTP 왕복이 요청 안에서 동기로 돌아
  ...
  (사용자 열거 방지) 결과를 기다릴 이유가 없었다. 부수로 타이밍 누출도 막힌다 —
  예전에는 "실제로 메일을 보낸 경우"만 몇 초 걸려서 **응답 시간만 재도 가입 여부**를 알 수 있었다.
...
- 실계정 검증: `passion@pusan.ac.kr`로 **실제 수신 확인**(Gmail SMTP). 요청→링크→변경→
  새 비밀번호 로그인→옛 비밀번호 401→토큰 재사용 400까지 전 구간 통과. pytest 464 passed.
