...
- **[perf] 도구 한 번에 수백 건의 SELECT가 나갔다.** `get_roadmap_items` 64건(base 21),
  3학기×6과목 배치 421건. 남은 학기 목록은 턴 안에서 안 변하는데 매번 이수기록 전체를
  스캔했고, pending 과목 학점을 매번 `db.get(Course)`로 다시 읽어 제안이 쌓일수록
  O(n²)였다(SELECT courses만 308회). 둘 다 턴 단위로 캐시해서 **28건 / 230건**으로 줄었다.
