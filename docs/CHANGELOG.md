...
- **[fix] `validate_timetable`이 같은 과목의 다른 분반을 통과시켰다 (심각).** 확률및통계
  ...
  있던 규칙이라 실제로는 새고 있었음).
- **[fix] 조합 랭킹이 학점 대신 요일 수를 먼저 봤다.** `timetable._rank_schedules`는
  "같은 과목 집합의 서로 다른 분반" 비교용이라 요일 수가 1순위다. 과목 수가 다른 조합에
  그대로 쓰니 **1과목(1일) 조합이 4과목(3일) 조합을 제쳤다** — 12학점이 가능한 후보 풀에서
  3학점짜리 단과목이 1·2위로 나왔다. `min(학점, 목표학점)`으로 묶어 정렬하는
  `_rank_built_combos`로 분리했다(목표 미달이면 학점 우선, 목표를 채웠으면 촘촘함 우선).
