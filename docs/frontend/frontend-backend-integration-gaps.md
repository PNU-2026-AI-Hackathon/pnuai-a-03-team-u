...
| 스키마/사용 불일치 | 6 | **M1·M2 수정 완료**, M3 부분 수정 |
...
### M2 — my.pusan 동기화 실패가 사용자에게 안 보인다 ✅ **수정 완료**

`PortalSyncResponse`는 `my_pusan_sso_ok`와 비교과/자격증/어학 카운터 6개를 돌려주는데
(`portal_sync.py:89-95`), **프론트 타입에 아예 없고 아무도 안 읽었다**.

백엔드는 my.pusan 크롤 실패를 의도적으로 삼키고 200을 돌려준다
(`portal_sync.py:146-153`, `212-216`) — 학지시(학적·성적)까지 같이 실패시키지 않으려는
설계다. 그래서 **"동기화 성공"이라고 안내하면서 비교과·자격증·어학은 하나도 안 들어온
상태**가 됐다.

**조치**: 7개 필드를 양쪽 프론트 타입에 추가하고, 문구·판정을 `portal.ts`에서 공유한다
(`MY_PUSAN_SYNC_FAILED_MESSAGE`, `isMyPusanSyncFailed`). 두 화면 모두 반영:

- **온보딩**: `my_pusan_sso_ok=false`면 배지가 "동기화 완료" → **"일부만 완료"**로 바뀌고,
  "확인하고 시작하기" 버튼 **위에** 안내를 띄운다(넘어가기 전에 읽도록).
- **InfoPage**: 동기화 폼 아래 경고 표시.

판정은 `=== false`로만 한다 — 필드가 없는 응답(구버전 백엔드·목)에서 `undefined`를
실패로 읽어 엉뚱한 경고를 띄우면 안 된다. 실제로 실행해서 확인했다:
정상 `false` / 실패 `true` / **필드 없음 `false`**.

### M3 — InfoPage가 sync 응답을 통째로 버린다 ⚠️ **부분 수정**

`InfoPage.tsx:356-363`은 목 모드가 아니면 `result`를 버리고 `refreshUser()` +
`window.location.reload()`만 했다. 결과:
- M2를 여기서도 못 알린다 → **해결**: `result`를 판정에 쓰고, 경고를 sessionStorage
  (`SYNC_WARNING_KEY`)에 적어 **리로드 너머로 전달**한 뒤 1회성으로 소비한다.
  리로드를 상태 갱신으로 바꾸는 건 InfoPage 파생 상태가 많아 회귀 위험이 커서
  **아래 잔여 작업으로 남긴다** — 경고 노출은 리로드를 유지한 채로 해결했다.
...
**결함은 소비 쪽에만 있었다** — M1(미리보기 빈 카드)과 M2(부분 실패 미노출). 둘 다 수정 완료.
...
| **P2** | `InfoPage.handleSync`의 `window.location.reload()`를 상태 갱신으로 교체. 파생 상태가 많아 회귀 위험이 있으니 단독으로 할 것 (경고 노출은 이미 해결됨) | `InfoPage.tsx:356` | ~2h |
