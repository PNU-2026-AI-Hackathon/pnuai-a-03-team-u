...
- 복수전공/부전공 요건 데이터, 입학연도별 요건, 전과 이력 모델링이 전부 미해결
  (위 "졸업요건 판정 엔진" 절 참고)
- **교양선택의 "영역" 단위 요건이 스키마에 아예 없음** (2026-07-06, 사용자가
  onestop.pusan.ac.kr 학사 규정 페이지로 확인): 학칙상 교양선택은 단순 학점 합계가
  아니라 "7개 영역(사상과역사/사회와문화/문학과예술/과학과기술/건강과레포츠/외국어/
  융복합) 중 최소 5개 영역에서 각 1과목 이상" 요건이다(교양필수 9학점은 별도).
  실제 학과 교육과정표는 이걸 "효원핵심교양"(=교양필수)/"효원균형교양"·"효원창의교양"
  (=교양선택, 영역별 구성)으로 브랜딩해서 쓴다. `RequirementCourse`/`RequirementCategory`
  둘 다 과목이 몇 번 영역인지 저장하는 컬럼이 없어서, 지금 엔진은 "교양선택 학점 합계"는
  셀 수 있어도 "5개 영역 이상 들었는지"는 원천적으로 판정 불가능하다. 영역 태그 컬럼/
  테이블 추가 + 엔진에 영역-카운트 로직이 필요 — 스키마 변경이라 별도 설계 논의 필요
