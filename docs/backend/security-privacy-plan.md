...
- **개인정보처리방침 + 수집 동의** — ✅ 구현 완료 (2026-08-24)
  ...
  재검증, 프론트만 믿지 않음) + `/privacy` 개인정보처리방침 페이지를 구현했다.
  방침 내용은 이 문서의 데이터 인벤토리(1절)와 `llm-privacy-audit.md`의 실측을
  그대로 근거로 썼다. 온보딩의 옛 "동의" 문구는 진짜 동의가 아니라 정적
  텍스트였던 걸 확인하고 포털 계정정보 미저장 사실만 남기게 정리했다.
  **아직 안 한 것**: 이 마이그레이션이 팀 공유 Supabase에는 아직 적용되지
  않았다 — Railway가 이미 배포 상태라 순서를 지켜서(Supabase 먼저) 반영해야
  한다, 안 그러면 그 사이 회원가입이 깨진다.
