...
- **개인정보처리방침 + 수집 동의** — ✅ 구현 완료, 배포 대기 (2026-08-24, PR
  ...
  **의도적으로 머지는 보류**: 이 마이그레이션을 팀 공유 Supabase에 아직
  적용하지 않았다. Railway가 이미 배포 상태라 순서를 지켜야 한다(Supabase
  먼저 → 확인 → 그다음 PR #215 머지) — 안 그러면 그 사이 회원가입이 깨진다.
  2026-08-24 세션에서는 "코드는 완성해서 남기되, 배포 타이밍은 팀이 직접
  판단해서 다음에 반영한다"로 결정했다. 다음 세션에서 이 PR부터 확인할 것.
