...
- **개인정보처리방침 + 수집 동의** — ✅ 구현 완료·배포 완료 (2026-08-24, PR
  [#215](https://github.com/PNU-2026-AI-Hackathon/pnuai-a-03-team-u/pull/215))
  "위탁 범위가 확정되는 배포 시점에 쓴다"고 미뤄뒀는데, 실제로는 이미 배포돼
  실사용자를 받고 있었다는 게 같은 세션에서 드러나서(README 1.3/F-08 정정과
  동일 계기) 더 미룰 수 없었다. `users.privacy_consent`/`privacy_consent_at`
  컬럼(마이그레이션 `b8d89abee0ab`) + 회원가입 필수 체크박스(서버 validator로도
  재검증, 프론트만 믿지 않음) + `/privacy` 개인정보처리방침 페이지를 구현했고,
  독립 리뷰로 코드 레벨 검증까지 마쳤다. 방침 내용은 이 문서의 데이터
  인벤토리(1절)와 `llm-privacy-audit.md`의 실측을 그대로 근거로 썼다. 온보딩의
  옛 "동의" 문구는 진짜 동의가 아니라 정적 텍스트였던 걸 확인하고 포털
  계정정보 미저장 사실만 남기게 정리했다.
  **⚠️ 인시던트 발생**: 머지를 보류하기로 했는데 팀원이 문서 전용 PR #216과
  함께 PR #215(코드)도 같이 머지해버려서, Supabase 마이그레이션 미반영 상태로
  Railway가 재배포돼 **회원가입이 잠깐 500으로 깨졌다.** 발견 즉시(`alembic
  stamp` + `upgrade head`) 복구, 실제 `/auth/signup` 재현 확인까지 마쳤다 —
  자세한 경위는 CHANGELOG 2026-08-24 인시던트 항목 참고.
