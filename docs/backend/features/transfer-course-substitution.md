...
| **트랙·복수전공 판정** | **반영되지 않는다.** `program_evaluator._resolve_completed_courses()`는 `StudentCourseRecord`만 이름 매칭한다. 그래서 같은 과목이 로드맵·시간표에서는 이수 완료로 빠지는데 트랙 진도에서는 미이수로 남는다 — 경로마다 다른 말을 하는 상태다. 배선하려면 그쪽도 `substituted_course_names`를 합쳐야 한다. |
