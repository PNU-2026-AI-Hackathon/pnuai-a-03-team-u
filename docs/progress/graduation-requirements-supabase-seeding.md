...
`requirement_text_rules` 876. `needs_review=false`(신뢰 가능)는 courses 기준 **317건뿐**
...
**11개**였다. 각 파일에서 `defaultTextSiteId`/`fnctNo`를 파싱하고, AIS API
(`/ais/getDeptInfoDeptList`, `/ais/UNIV/{collegeCode}/getDeptInfoMajorList`)로 학과 코드를
찾은 뒤, 같은 URL을 최대 8회 재시도하는 스크립트로 **9개를 실제 데이터로 복구**했다
(경영학과·고고학과·무용학과·식품영양학과·아동가족학과·음악학과·의류학과·조경학과 성공,
조형학과는 20회 넘게 재시도해도 계속 빈 응답이라 실패 — 이 학과는 그 시점에 AIS 쪽에 데이터
자체가 없었을 가능성).
...
| `needs_review=false` | 4,631 | 317 |
| 카탈로그 전용 의존 학과 | 45 | 36 (9개 복구) |

### 남은 일

- 카탈로그 전용 학과 36개 중 나머지도 같은 방식(AIS 위젯 재시도, 또는 다른 원문 검색)으로
  더 복구할 여지가 있다. 이번엔 "로컬에 빈 AIS 파일이 이미 있는 경우"만 처리했고, 애초에
  원문 시도조차 없었던 학과는 손대지 않았다.
