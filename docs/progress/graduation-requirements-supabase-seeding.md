...
## 카탈로그 추정 데이터 검토 상태 수정 + 원문 복구 (2026-07-03)

### 문제

사용자가 엑셀 리포트에서 경제학부를 직접 보다가 "전공선택에 일반선택까지 섞인 것 같다"고
지적했다. 확인해보니 경제학부 데이터는 학과 공식 졸업요건 문서에서 온 게 아니라
**`department_courses_from_catalog`** 소스였다 — 학과의 실제 교육과정표를 찾아 파싱한 게
아니라, 수강편람 카탈로그에 "이 학과가 개설한다"고 찍힌 과목을 전부 모아서 카탈로그
자체의 교과목구분(전공필수/전공선택/일반선택 등) 태그를 그대로 가져다 쓴 것이다. 여러
학기 카탈로그 스냅샷을 섞어 쓰다 보니 연도별로 다르게 찍힌 태그가 섞일 수 있다.

전수 확인 결과 **106개 학과 x 구분 조합, 4,314개 과목 행**이 이 소스를 쓰고 있었고, 그 중
**45개는 이게 유일한 소스**였다. 문제는 이 4,314건이 전부 `needs_review=false`(검토완료)로
표시돼 있었다는 것 — 실제로는 공식 문서 검증이 전혀 안 된 데이터인데 "신뢰 가능"으로 잘못
분류돼 있었다.

### 조치 1: needs_review 정정

`build_graduation_requirement_seed_tables.py`의 `build_course_rows()`에서
`source_table == "department_courses_from_catalog"`인 행은 무조건 `needs_review=true`로
강제하고, review_reason에 이유를 남기도록 고쳤다. 재시딩 결과 `needs_review=false` 행이
4,631 -> 317건으로 줄었다 (데이터가 사라진 게 아니라 잘못된 "검증완료" 표시를 없앤 것).

### 조치 2: 로컬에 이미 있던 원문 재복구

45개 카탈로그 전용 학과의 로컬 소스 폴더를 까보니 두 부류가 섞여 있었다:

1. 진짜 원문이 없는 경우 (예: 경제학부 — 학부장인사말/교육목표/연혁 페이지만 있음)
2. **원문은 있는데 파이프라인이 못 쓴 경우** — 부산대 여러 단과대 홈페이지가 쓰는 AIS
   동적 위젯(`fnctId=curriculum`, `/curriculum/{siteId}/{fnctNo}/view` POST)이 크롤링
   당시 "등록된 데이터가 없습니다"라는 빈 응답을 줘서, 크롤러가 그 빈 스냅샷을 그대로
   저장해버린 경우. 이 위젯은 플레이키해서 같은 요청을 여러 번 재시도하면 실제 데이터가
   나오기도 한다.

45개 중 이 패턴(로컬에 `fnctId=curriculum` + "없습니다" 파일이 이미 있음)에 해당하는 학과가
**11개**였다(경영학과 포함, 분자생물학과·식품공학과는 로컬 파일이 대학원 페이지라 제외).
각 파일에서 `defaultTextSiteId`/`fnctNo`를 파싱하고, AIS API
(`/ais/getDeptInfoDeptList`, `/ais/UNIV/{collegeCode}/getDeptInfoMajorList`)로 학과 코드를
찾은 뒤, 같은 URL을 최대 8회 재시도하는 스크립트로 **8개를 실제 데이터로 복구**했다
(경영학과·고고학과·무용학과·식품영양학과·아동가족학과·음악학과·의류학과·조경학과 성공,
조형학과는 계속 빈 응답이라 실패 — 이 학과는 그 시점에 AIS 쪽에 데이터 자체가 없었을 가능성).

예: 경영학과 원문에서 확인한 사실 — "2026학년도 교육과정부터 적용: 전공선택 21->18학점,
일반선택 15->18학점" — 카탈로그 추정으로는 절대 알 수 없는, 학과가 명시적으로 정한 수치다.

### 조치 3: 경제학부 "펜토미노 교육과정" (사용자 제보)

경제학부는 로컬에 AIS 빈 파일도 없었다 — 학부장인사말/교육목표/연혁 페이지뿐이라
자동 탐색으로는 원천적으로 못 찾는 상태였다. 사용자가 직접 경제학부 홈페이지에서
"학부 > 펜토미노 교육과정" 메뉴를 확인해 알려줘서 찾았다. 이 메뉴는 표가 아니라
`fnctId=docViewer` 위젯(PDF 뷰어)이었고, `/docViewer/{siteId}/{fnctNo}/{fileId}/fileDownload.do`
로 실제 PDF(`펜토미노_교육과정_2026_경제학부.pdf`)를 받을 수 있었다.

PDF는 "모듈-트랙" 구조의 인포그래픽이라 `pdftotext`로 뽑으면 열이 뒤섞여서 일반
파이프라인(표 파싱)으로는 자동 추출이 안 됐다 (`course_candidate_count: 0`). 대신 모듈
구성표를 직접 읽어서 확인한 결과, 필수모듈(경제학 기초이론/기초분석기법)의 핵심 6과목
— 경제학원론(I)/(II)(전공기초), 미시경제학/거시경제학/경제통계학/경제수학(전공필수) —
이 기존 카탈로그 추정 데이터와 **정확히 일치**한다는 걸 확인했다. 이 6건만
`requirement_course_corrections.csv`에 `confirm`으로 기록했다 (트랙별 전공선택 세부
분류는 졸업요건 판정에 필요 없다는 사용자 확인에 따라 생략).
...
| `needs_review=false` | 4,631 -> 317 | 323 |
| 카탈로그 전용 의존 학과 | 45 | 37 (8개 완전 복구 + 경제학부 핵심 6과목 확인) |

### 남은 일

- 카탈로그 전용 학과 37개 중 나머지도 같은 방식(AIS 위젯 재시도, `docViewer`/PDF 탐색,
  또는 다른 원문 검색)으로 더 복구할 여지가 있다. 이번엔 "로컬에 빈 AIS 파일이 이미 있는
  경우"만 우선 처리했고, 애초에 원문 시도조차 없었던 나머지 학과는 손대지 않았다.
- 복구된 8개 학과도 옛 카탈로그 추정 행이 완전히 삭제되지 않고 새 원문 데이터와 같이
  남아있다 (둘 다 needs_review=true 상태). 사람 검토 시 중복/구버전 정리 필요.
