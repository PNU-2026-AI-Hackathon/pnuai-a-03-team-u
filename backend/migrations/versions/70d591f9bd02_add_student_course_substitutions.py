"""편입 전적대 이수기록 ↔ 대체된 PNU 과목 조인 테이블(student_course_substitutions)
...
**학생 본인이 화면에서 고른 값만** 담는다.

컬럼 하나가 아니라 별도 테이블인 이유는 관계가 N:M이기 때문이다. 전적대
`교양선택 15학점` 한 줄은 개별 과목이 아니라 **여러 세부영역**(`courses`의 `ZFz…`
placeholder 행)을 채운 것으로 인정받고, 반대로 전적대 두 과목이 PNU 한 과목을
대체하는 경우도 있다.

학점 컬럼은 손대지 않는다 — 전적대 학점은 이수기록 행에 그대로 있고 졸업요건 엔진은
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # 이수기록이 지워지면 대체 관계도 같이 사라져야 한다. 성적표 재동기화로
        ...
        sa.UniqueConstraint("record_id", "course_id", name="uq_student_course_substitution"),
    )
    op.create_index(
        op.f("ix_student_course_substitutions_record_id"),
        "student_course_substitutions",
        ["record_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_student_course_substitutions_course_id"),
        "student_course_substitutions",
        ["course_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_student_course_substitutions_course_id"),
        table_name="student_course_substitutions",
    )
    op.drop_index(
        op.f("ix_student_course_substitutions_record_id"),
        table_name="student_course_substitutions",
    )
    op.drop_table("student_course_substitutions")
