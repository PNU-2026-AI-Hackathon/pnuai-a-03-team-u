...
class Settings(BaseSettings):
    ...
    # 로그에 링크도 안 남는 반쪽 상태가 된다(2026-08-20 실측). 그런 조합을 **만들기
    # 어렵게** 하려는 것이지 원천 차단은 아니다 — SMTP_*를 직접 쓰는 길이 열려 있는 한
    # 반쪽 설정은 여전히 가능하다. 그래서 `resolve_smtp()`가 비밀번호를 RESEND_API로
    # 폴백하고, `startup_log()`가 자격증명이 빈 상태를 기동 시 경고한다.
