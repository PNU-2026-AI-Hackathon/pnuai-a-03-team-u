...
def _build_student_context_block(db: Session, user: User) -> str:
    ...
    completed_by_liberal_area: dict[str, list[StudentCourseRecord]] = {}
    for r in completed:
        ...
        liberal_area = r.liberal_area or (r.category if r.category in _BALANCED_LIBERAL_AREAS else None)
        if liberal_area:
            completed_by_liberal_area.setdefault(liberal_area, []).append(r)
    ...
    # 균형교양 세부영역별 이수/미이수 요약. portal_sync가 One-Stop 학교 판정을
    # liberal_area에 저장한 rows만 집계한다.
    balanced_lines: list[str] = []
    missing_areas: list[str] = []
    for area in _BALANCED_LIBERAL_AREAS:
        recs = completed_by_liberal_area.get(area)
        if recs:
