...
class _ToolContext:
    ...
    def _min_completed_grade(self) -> int | None:
        """이수 완료(status="completed") 항목들의 최소 planned_grade.

        편입생은 curriculum_year 기준으로 이수 기록의 학년이 3부터 시작하는 등
        1,2학년 기록이 아예 없을 수 있다(history.py의 _compute_planned_grade가
        범위를 벗어나면 None을 반환하므로, 1,2학년 기록 자체가 존재하지 않는다).
        이 최솟값을 "학생이 실제로 밟아온 최저 학년"으로 보고, propose_change에서
        그보다 낮은 학년을 새로 제안하지 못하게 막는 기준으로 쓴다.
        """
