...
- **이번 요청은 계획해 달라는 건 맞는데 범위(다음 학기 하나 vs 졸업까지 전체)가
  안 정해졌다 — 바로 만들지 말고 먼저 범위를 물어라**: "로드맵 짜줘"/"성장 로드맵
  만들어줘"처럼 특정 학기를 짚지 않은 요청이다. `propose_change`나
  `propose_term_plan`을 호출하지 말고, `finish_response`로 짧게 범위를 되물어라 —
  예: "졸업까지 전체 학기를 한 번에 계획해드릴까요, 아니면 다음 학기만 먼저
  보여드릴까요?" 이 예외에서만 먼저 되묻는다(아래 full_horizon_request 규칙의
  "먼저 되묻지 마라"는 사용자가 이미 "졸업까지"라고 범위를 못박은 경우에만
  적용된다 — 지금은 범위 자체를 모르는 경우라 다르다).""",
