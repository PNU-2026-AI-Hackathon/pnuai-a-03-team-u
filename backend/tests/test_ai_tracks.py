...
_TABLES = [
    ...
    CourseRoadmap.__table__, Course.__table__,
]
...
class GetAvailableTracksToolTest(unittest.TestCase):
    ...
    def test_공통교과목_목록을_함께_돌려준다(self):
        ...
        result = self._ctx(18).get_available_tracks()
        names = [c["course_name"] for c in result["ai_common_courses"]]
    ...
    def test_공통교과목은_일반선택만_매칭한다(self):
        ...
        self.db.flush()
        by_name = {c["course_name"]: c for c in self._ctx(18).get_available_tracks()["ai_common_courses"]}
        self.assertFalse(by_name["데이터 마이닝"]["offered"], "전공선택 동명 과목을 집으면 안 된다")
        self.assertTrue(by_name["AI리터러시의이해"]["offered"])
