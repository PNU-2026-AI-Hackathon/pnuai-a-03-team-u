...
# 계층 FK 해석용 학과명 -> departments.id 캐시. "교양교육원"처럼 departments에
# 없는 개설 주체는 컨벤션대로 department_id=None으로 둔다.
...
def setup_global_requirements(db):
    # 프로그램 마스터 — 요건세트 룩업 키(academic_program_code)의 FK 대상
    db.add_all([
        AcademicProgram(academic_program_code="CS01", program_name="컴퓨터공학과"),
        AcademicProgram(academic_program_code="MATH01", program_name="수학과"),
        AcademicProgram(academic_program_code="CS02", program_name="컴퓨터공학과"),
        AcademicProgram(academic_program_code="CS03", program_name="컴퓨터공학과"),
        AcademicProgram(academic_program_code="MATH02", program_name="수학과"),
    ])
    db.commit()

    cs_dept_id = DEPT_IDS["컴퓨터공학과"]
    math_dept_id = DEPT_IDS["수학과"]

    # Requirement Sets — 타학과 과목 필터가 FK 비교만 하므로 department_id를 채운다
    ...
    # TC09/TC10 전용: 최소전공 총학점(minimum_major_total) 집계 판정.
    # 텍스트 규칙에서 승격한 "복수전공 총 N학점" 형태의 요건을 검증한다.
    math_total_req = RequirementSet(
        academic_program_code="MATH02",
        department_id=math_dept_id,
        program_type="dual",
        curriculum_year="2026",
        is_active=True,
    )
    db.add(math_total_req)
    db.commit()
    math_total_cat = RequirementCategory(external_id="math_total1", requirement_set_id=math_total_req.id, category_code="minimum_major_total", category_name="최소전공(복수)", minimum_credits="21", rule_type="minimum_credits", needs_review=False)
    db.add(math_total_cat)
...
def run_golden_tests():
    ...
    for scenario in GOLDEN_SCENARIOS:
        ...
        # 2. Programs — 골든 데이터의 "major"(학과명)는 main 계층에서 department_id로 해석
        for p in scenario["programs"]:
            db.add(UserAcademicProgram(
                user_id=user.id,
                academic_program_code=p["code"],
                program_type=p["type"],
                department_id=DEPT_IDS.get(p["major"]),
                curriculum_year="2026",
                status="active"
            ))

        # 3. Courses — 골든 데이터의 "department"(개설 학과명)는 department_id로 해석.
        #    "교양교육원"처럼 departments에 없는 주체는 None(컨벤션).
        for c in scenario["courses"]:
            course_key = (c["name"], c["department"])
            if course_key not in course_cache:
                new_course = Course(course_name=c["name"], department_id=DEPT_IDS.get(c["department"]))
                db.add(new_course)
                db.commit()
