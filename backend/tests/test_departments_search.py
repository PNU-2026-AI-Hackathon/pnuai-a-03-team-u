...
SW융합트랙·SW연계전공은 소속 학과가 아니라 기존 학과 위에 얹는 부가 과정이다.
"내 학과"를 고르는 자리에 20개 넘게 섞여 나오면 정작 찾으려는 학과가 묻힌다.

핀테크융합전공 같은 "OO융합전공"은 다르다 — 그 자체가 독립된 모집단위(자체
department 행)라 부가 과정이 아니라 주전공이 될 수 있어서(2026-08-25 확인,
과목·시간표 스코프 정상 동작) 더 이상 안 감춘다.

여기서 지키려는 선은 하나다: 감추는 건 "기존 학과 위에 얹는" 부가 과정뿐이고,
이름에 "융합"이 들어가는 정식 학과(의생명융합공학부 등)나 독립 모집단위인
융합전공은 반드시 남아야 한다.
"""
...
class DepartmentSearchTest(unittest.TestCase):
    ...
    def test_융합전공_학과는_후보에_남는다(self):
        """독립 모집단위라 부가 과정과 다르게 자동완성에 계속 나와야 한다."""
