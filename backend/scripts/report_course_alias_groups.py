...
- **단, 2026-08-20 정정**: 컴퓨터공학전공의 `일반물리학`(CB1501005/CB1501009)은 정상
  중복이 아니라 시드 CSV 수기 입력에서 로마숫자 접미사가 빠져 생긴 **적재 데이터 오류**였다.
  교정으로 alias 그룹은 **5개 15행 → 4개 13행**이 됐다(줄어든 건 이 한 그룹뿐).
  `이산수학`(CB1501027/CB2001104)도 같은 유실이었지만 **alias 그룹이었던 적은 없다** —
  그룹 키에 `category`가 들어가는데 두 행은 전공기초/전공선택으로 갈리기 때문이다.
  구분 기준은 교과목코드 접두사다 — 접두사가 같은데 과목명이 겹치면 접미사 유실을 의심하라
  (`import_courses_from_ais.find_suffix_dropped_collisions`가 적재 시점에 막는다).
  ⚠️ 다만 그 검사는 `ais_dept_code`로 묶으므로 **교양 행(코드 기준 전역 dedup)은 못 잡는다.**
  배포 시드 전수 기준 "**한 접두사 안에서** 코드가 갈리는" 교양 사례는 **2건**이다:

      대학영어         ZE1000113(98개 학과) / ZE1000116(1개)
      공학작문및발표    ZE1000043(23개) / ZE1000118(1) / ZE1000119(1)   ※ CB·DM도 있으나 별개

  둘 다 "주 코드 하나 + 학과 1곳짜리 아웃라이어" 패턴이라 접미사 유실 후보다. 두 코드를
  함께 쓰는 학과가 없어 검사를 통과한다(알려진 한계, 후속 과제).
  판정할 때 그룹 전체의 접두사 집합을 보면 안 된다 — `공학작문및발표`는 CB/DM/ZE 3종이라
  "접두사가 갈리니 정상"으로 오판하기 쉽다. **접두사별로 나눠서** 그 안에 코드가 여럿인지
  봐야 한다(2026-08-20 3차 리뷰가 이 오판을 잡았다).
...
#
# 이수구분·학점까지 넣는 이유: 같은 (과목명, 학과, 전공)인데 이수구분이 갈리는 항목이
# 실존한다(예: 생활과통계 ZF1100694 기초교양 / ST1100694 효원균형교양). 졸업요건 집계가
# 이수구분 기준이라 합치면 판정이 달라진다.
# 반대로 학과·전공을 빼면 남의 학과 분반이 섞인다(일반물리학(I)은 31개 학과에 존재).
#
# 주의 — 2026-08-20 정정: 예전 주석은 컴퓨터공학전공의 `이산수학` 두 행(CB1501027 전공기초 /
# CB2001104 전공선택)을 이 키가 필요한 근거로 들었는데, 그건 원본 성질이 아니라 **시드 CSV의
# 접미사 유실 버그**였다(원문은 `이산수학(I)`/`이산수학(II)`). 교정 후 두 행은 과목명부터
# 다르므로 애초에 같은 그룹이 아니다. 자세한 경위는
# scripts/fix_truncated_course_names_2026_08_20.py 참고.
...
# 예: 교양 생활과통계가 ZF1100694(기초교양)와 ST1100694(효원균형교양) 두 항목으로 존재.
_INCONSISTENT_QUERY = """
SELECT
